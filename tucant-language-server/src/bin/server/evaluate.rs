...
pub trait Value<'a>: Debug {
    fn evaluate_call(self: Rc<Self>, context: &mut Vec<(String, Rc<dyn Value<'a> + 'a>)>, args: &[Span<'a, Ast<'a>>]) -> EvaluateResult<'a, Rc<dyn Value<'a> + 'a>> {
        Err(EvaluateError { location: None, reason: format!("not yet implemented").into() }) // TODO FIXME add span information
    }
...
pub trait Type<'a>: Debug {
    fn typecheck_call(self: Rc<Self>, context: &mut Vec<(String, Rc<dyn Type<'a> + 'a>)>, args: &[Span<'a, Ast<'a>>]) -> EvaluateResult<'a, Rc<dyn Type<'a> + 'a>> {
        Err(EvaluateError { location: None, reason: format!("not yet implemented").into() })
    }
...
impl<'a> Value<'a> for AddLambdaValue {
    fn evaluate_call(self: Rc<Self>, context: &mut Vec<(String, Rc<dyn Value<'a> + 'a>)>, args: &[Span<'a, Ast<'a>>]) -> EvaluateResult<'a, Rc<dyn Value<'a> + 'a>> {
        ...
        let left_value = left_value.downcast_integer_value().ok_or(EvaluateError { location: None, reason: format!("expected integer type, got {:?}", left_value).into()})?;
        let right_value = right_value.downcast_integer_value().ok_or(EvaluateError { location: None, reason: format!("expected integer type, got {:?}", right_value).into()})?;
        Ok(Rc::new(IntegerValue(left_value.0.checked_add(right_value.0).ok_or(EvaluateError { location: None, reason: format!("integer overflow, adding {:?} and {:?}", left_value, right_value).into()})?)))
    }
}
...
impl<'a> Type<'a> for AddLambdaType {
    fn typecheck_call(self: Rc<Self>, context: &mut Vec<(String, Rc<dyn Type<'a> + 'a>)>, args: &[Span<'a, Ast<'a>>]) -> EvaluateResult<'a, Rc<dyn Type<'a> + 'a>> {
        ...
        let right_value = right_value.downcast_integer_type().ok_or(EvaluateError { location: None, reason: format!("expected integer type, got {:?}", right_value).into()})?;
        Ok(Rc::new(IntegerType(left_value.0.and_then(|l| right_value.0.map(|r| l.checked_add(r).ok_or(EvaluateError { location: None, reason: format!("integer overflow, adding {:?} and {:?}", left_value, right_value).into()}))).transpose()?)))
    }
}
...
pub fn evaluate_with_context<'a>(context: &mut Vec<(String, Rc<dyn Value<'a> + 'a>)>, value: Span<'a, Ast<'a>>) -> EvaluateResult<'a, Rc<dyn Value<'a> + 'a>> {
    match value.inner {
