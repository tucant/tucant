...
import {ModuleMenuType, ModulesResponseType, ModuleType} from "../validation-io-ts";

type ModuleListProps = {listData: ModulesResponseType};
export function ModuleList({ listData }: ModuleListProps) {
    return (
        <List>
            { listData.Submenu && listData.Submenu.map((submenuEntry: ModuleMenuType) => (
                    <RouterLink
            ...
            { listData.Modules && listData.Modules.map((module: ModuleType) => (
                    <RouterLink
                        key={module.tucan_id}
