...
pub fn vorlesungsverzeichnis<TucanType: Tucan + 'static>(
    VorlesungsverzeichnisProps { vv }: &VorlesungsverzeichnisProps,
) -> Html {
    let handler = async |tucan: RcTucanType<TucanType>,
                         current_session,
                         revalidation_strategy,
                         additional| {
        tucan
            .0
            .vv(Some(&current_session), revalidation_strategy, additional)
            .await
    };
