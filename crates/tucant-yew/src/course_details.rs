    ...
    Ok(html! {
        <div class="container">
            ...
            if let Some(course) = data {
                <CourseDetailsViewer course={course.to_owned()} reload={reload} />
            }
        </div>
    })
}
...
#[derive(Properties, PartialEq)]
pub struct CourseDetailsViewerProps {
    pub course: CourseDetailsResponse,
    pub reload: Callback<MouseEvent>,
}

#[function_component(CourseDetailsViewer)]
pub fn course_details_viewer(CourseDetailsViewerProps { course, reload }: &CourseDetailsViewerProps) -> Html {
    html! {
        <div>
