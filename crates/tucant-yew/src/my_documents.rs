...
pub fn my_documents<TucanType: Tucan + 'static>() -> HtmlResult {
        ...
        use_effect_with((), move |()| {
            if let Some(current_session) = (*current_session_handle).to_owned() {
                ...
                spawn_local(async move {
                    match tucan.0.my_documents(&current_session, RevalidationStrategy { max_age: 14 * 24 * 60 * 60, invalidate_dependents: Some(true) }).await {
                        Ok(response) => {
                            data.set(Ok(Some(response)));
                            loading.set(false);

                            match tucan.0.my_documents(&current_session, RevalidationStrategy { max_age: 4 * 24 * 60 * 60, invalidate_dependents: Some(true) }).await {
    ...
    let reload = {
        ...
        Callback::from(move |e: MouseEvent| {
            if let Some(current_session) = (*current_session).to_owned() {
                ...
                let loading = loading.clone();
                spawn_local(async move {
                    match tucan.0.my_documents(&current_session, RevalidationStrategy { max_age: 0, invalidate_dependents: Some(true) }).await {
                        Ok(response) => {
                            data.set(Ok(Some(response)));
                            loading.set(false);
                        }
