...
async fn logout_endpoint(jar: CookieJar) -> Result<impl IntoResponse, TucanError> {
    let tucan = TucanConnector::new().await?;
...
async fn course_details_endpoint(
    jar: CookieJar,
    Path(module): Path<String>,
) -> Result<impl IntoResponse, TucanError> {
    let tucan = TucanConnector::new().await?;
...
async fn after_login_endpoint(jar: CookieJar) -> Result<impl IntoResponse, TucanError> {
    let tucan = TucanConnector::new().await?;
