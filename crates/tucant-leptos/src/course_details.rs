        ...
        view! {
            <div>
                ...
                {match (course.teilnehmer_min, course.teilnehmer_max) {
                    ...
                    (None, Some(max)) => {
                        view! { <div>{format!("Maximal {max} Teilnehmende")}</div> }.into_any()
                    }
                    (Some(min), None) => {
                        view! { <div>{format!("Mindestens {min} Teilnehmende")}</div> }.into_any()
                    }
                    (Some(min), Some(max)) => {
                        view! { <div>{format!("{min} - {max} Teilnehmende")}</div> }.into_any()
                    }
                }}
                <h2>{"Übungsgruppen"}</h2>
                <table class="table">
                    ...
                    <tbody>
                        ...
                        {course
                            ...
                            .map(|uebungsgruppe| {
                                view! {
                                    <tr class=if uebungsgruppe.active {
                                        "table-primary"
                                    } else {
                                        ""
                                    }>
                                        <th scope="row">
                                            <a href=format!(
                                                "/course-details/{}",
                                                uebungsgruppe.url.clone(),
                                            )>{uebungsgruppe.name.clone()}</a>
                                        </th>
                                        <td>
                                            {uebungsgruppe.date_range.clone().unwrap_or_default()}
                                        </td>
                                        <td>{uebungsgruppe.uebungsleiter.clone()}</td>
                                    </tr>
                                }
                                    .into_any()
                            })
                    ...
                    <tbody>
                        {course
                            ...
                            .map(|anmeldefrist| {
                                view! {
                                    <tr>
                                        <td>{anmeldefrist.zulassungstyp.clone()}</td>
                                        <td>{anmeldefrist.block_type.clone()}</td>
                                        ...
                                        .map(|termin| {
                                            view! {
                                                <tr>
                                                    ...
                                                    <td>{termin.time_start.clone()}</td>
                                                    <td>{termin.time_end.clone()}</td>
                ...
                <h2>{"Termine Plenumsveranstaltung"}</h2>
                <table class="table">
                    <thead>
                        <tr>
                            <th scope="col">{"Datum"}</th>
