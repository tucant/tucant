...
pub fn MyCourses() -> impl IntoView {
    ...
    let handler = async |tucan: Arc<ApiServerTucan>, current_session, revalidation_strategy, additional: SemesterId| tucan.my_courses(&current_session, revalidation_strategy, additional).await;

    let navigate = leptos_router::hooks::use_navigate();

    use_authenticated_data_loader(handler, Signal::derive(semester), 14 * 24 * 60 * 60, 60 * 60, move |my_modules: MyCoursesResponse, reload| {
        ...
        view! {
            <div>
                <h1>
                    {"Meine Veranstaltungen"} {" "} <button type="button" class="btn btn-light">
                                    ...
                                    <tbody>
                                        {section
                                            ...
                                            .map(|course| {
                                                view! {
                                                    <tr>
                                                        <th scope="row">{course.nr.clone()}</th>
                                                        <td>
                                                            <a href=format!(
                                                                "/course-details/{}",
                                                                course.url.clone(),
                                                            )>{course.title.clone()}</a>
                                                        </td>
                                                        <td>{course.date_range.clone()}</td>
                                                        <td>{course.location.clone()}</td>
                                                    </tr>
                                                }
                                            })
                                            .collect::<Vec<_>>()}
