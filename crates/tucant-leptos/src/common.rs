...
pub fn use_authenticated_data_loader<I: Send + Sync + Clone + PartialEq + 'static, O: Clone + 'static>(handler: impl AsyncFn(Arc<ApiServerTucan>, LoginResponse, RevalidationStrategy, I) -> Result<O, TucanError> + Copy + 'static, request: Signal<I>, cache_age_seconds: i64, max_stale_age_seconds: i64, render: impl Fn(O, Callback<MouseEvent>) -> AnyView + Send + 'static) -> AnyView {
...
pub fn use_unauthenticated_data_loader<I: Send + Sync + Clone + PartialEq + 'static, O: Clone + 'static>(handler: impl AsyncFn(Arc<ApiServerTucan>, Option<LoginResponse>, RevalidationStrategy, I) -> Result<O, TucanError> + Copy + 'static, request: Signal<I>, cache_age_seconds: i64, max_stale_age_seconds: i64, render: impl Fn(O, Callback<MouseEvent>) -> AnyView + Send + 'static) -> AnyView {
    use_data_loader(false, handler, request, cache_age_seconds, max_stale_age_seconds, render)
}

fn use_data_loader<I: Send + Sync + Clone + PartialEq + 'static, O: Clone + 'static>(authentication_required: bool, handler: impl AsyncFn(Arc<ApiServerTucan>, Option<LoginResponse>, RevalidationStrategy, I) -> Result<O, TucanError> + Copy + 'static, request: Signal<I>, cache_age_seconds: i64, max_stale_age_seconds: i64, render: impl Fn(O, Callback<MouseEvent>) -> AnyView + Send + 'static) -> AnyView {
    ...
    let data = {
        LocalResource::new(move || {
            ...
            async move {
                match handler(tucan.clone(), session.get(), RevalidationStrategy { max_age: cache_age_seconds, invalidate_dependents: Some(true) }, request.get()).await {
                    Ok(response) => {
                        return Ok(response);
