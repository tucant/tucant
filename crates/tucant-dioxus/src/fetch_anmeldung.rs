...
pub fn FetchAnmeldung() -> Element {
    let handler = async |tucan: RcTucanType, current_session: LoginResponse, revalidation_strategy, (): ()| {
        let anmeldung_response = tucan.anmeldung(current_session.clone(), revalidation_strategy, AnmeldungRequest::default()).await.unwrap();
        let mut output = Vec::new();
        for course_of_study in anmeldung_response.studiumsauswahl {
            let result = recursive_anmeldung(&tucan.0, &current_session, course_of_study.value.clone()).await;
    ...
    let result = use_authenticated_data_loader(handler, ReadSignal::new(Signal::new(())), 14 * 24 * 60 * 60, 60 * 60, |output: Vec<(String, Vec<u8>)>, _reload| {
        rsx! {
            for entry in output {
                a {
                    href: {
