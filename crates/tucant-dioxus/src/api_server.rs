...
impl Tucan for ApiServerTucan {
    async fn login(&self, request: LoginRequest) -> Result<LoginResponse, TucanError> {
        let response = self.client.post("http://localhost:3000/api/v1/login").json(&request).send().await?.error_for_status()?.json().await?;
        Ok(response)
    }
    ...
    async fn logout(&self, _request: &LoginResponse) -> Result<(), TucanError> {
        self.client.post("http://localhost:3000/api/v1/logout").send().await?.error_for_status()?;
