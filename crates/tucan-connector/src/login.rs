...
pub async fn logout(connector: &TucanConnector, login_response: &LoginResponse) -> Result<(), TucanError> {
...
pub async fn login(client: &MyClient, login_request: &LoginRequest) -> Result<LoginResponse, TucanError> {
    static NEXT_URL_REGEX: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"0; URL=/scripts/mgrqispi\.dll\?APPNAME=CampusNet&PRGNAME=STARTPAGE_DISPATCH&ARGUMENTS=-N(?P<id>\d+),-N000019,-N000000000000000").unwrap());
